...
def get_rental_price(rentals, section_name, item_name):
    for section in rentals.get("rentals", {}).get("sections", []):
        if section.get("name", "").lower() != section_name:
            continue
        for item in section.get("items", []):
            ...
            if price:
                return parse_price(price[0].get("value"))
    return None
...
def build_resort(static_info, rates, rentals, weather):
    ...
    prices = {
        "dayTicketPrice": get_rate_price(rates, "adult", "peak_online"),
        "skiRentalPrice": get_rental_price(rentals, "rental_daily_packages", "all ages"),
        "lessonsPrice": get_rental_price(rentals, "rental_lesson_packages", "lesson package – ski"),
    }
    prices["snowBoardRentalPrice"] = prices["skiRentalPrice"]
    for field, value in prices.items():
        if value is not None:
            resort[field] = value
    resort["hasLessons"] = prices["lessonsPrice"] is not None
    return resort
