    ...
    if isinstance(row, dict):
        name_key = next(
            (
                key
                for key in row
                if key.lower() in {"name", "item", "description"}
                or "pricing" in key.lower()
                or "package" in key.lower()
                or "product" in key.lower()
                or "item" in key.lower()
            ),
            next(iter(row), "name"),
        )
        return {
...
def to_rates_dto(rates: list[Any]) -> dict[str, Any]:
...
def to_weather_dto(status: dict[str, Any]) -> dict[str, Any]:
    ...
    conditions = {
        ...
        for value in [status.get("snow_conditions", {}).get(source)]
        if value not in (None, "")
    }
