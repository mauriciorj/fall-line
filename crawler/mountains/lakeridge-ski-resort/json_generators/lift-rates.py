"""Lake Ridge lift-ticket rates crawler."""
...
def get_lift_rates():
    ...
    result = {}
    for title in [
        "Beginner Area Ticket Prices",
        "Monday to Wednesday Lift Tickets",
        "Thursday to Sunday Lift Tickets",
    ]:
        heading = soup.find("h2", string=lambda text: text and title.lower() in text.lower())
        if heading:
            table = heading.find_next("table")
            rows = extract_table_data(table) if table else []
            if rows:
                result[title] = rows
    return result
