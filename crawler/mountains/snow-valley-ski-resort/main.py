...
SNOW_VALLEY_DIR = Path(__file__).resolve().parent
CRAWLER_DIR = SNOW_VALLEY_DIR.parents[1]
SNAPSHOT_GENERATOR = SNOW_VALLEY_DIR / "snapshot_generator.py"
SNAPSHOT_FILE = SNOW_VALLEY_DIR / "snapshot_generator.json"
TABLE_NAMES = ("resorts", "resortHours", "resortRates", "resortRentals", "weatherAndStatus")
METADATA_FIELDS = {"resortId", "sourceUrl", "updatedAt"}

sys.path.insert(0, str(CRAWLER_DIR))
from convex_client import push_resort_hours, push_resort_rates, push_resort_rentals, push_ski_resorts, push_weather_and_status


def run_snapshot_generator():
    subprocess.run([sys.executable, str(SNAPSHOT_GENERATOR)], cwd=SNOW_VALLEY_DIR, check=True)
...
def get_table_contents(snapshot):
    ...
    for table in tables:
        ...
        if name in contents:
            raise ValueError(f"Duplicate table in snapshot: {name}")
        content = table.get("content")
        if not isinstance(content, dict):
            raise ValueError(f"Table content must be an object: {name}")
...
def push_snapshot(snapshot):
    ...
    if not isinstance(resort_id, str) or not resort_id or snapshot.get("resortId") != resort_id:
        raise ValueError("Snapshot resort ID mismatch")
    metadata = {name: require_metadata(contents[name], name) for name in TABLE_NAMES[1:]}
    ...
    hours, rates, rentals, weather = (contents[name] for name in TABLE_NAMES[1:])
    ...
    return {
        ...
        "weatherAndStatus": push_weather_and_status(weather_id, weather_url, {key: value for key, value in weather.items() if key not in METADATA_FIELDS}, updated_at_ms=weather_time),
    }
