...
BRIMACOMBE_DIR = Path(__file__).resolve().parent
CRAWLER_DIR = BRIMACOMBE_DIR.parents[1]
SNAPSHOT_GENERATOR = BRIMACOMBE_DIR / "snapshot_generator.py"
SNAPSHOT_FILE = BRIMACOMBE_DIR / "snapshot_generator.json"
TABLE_NAMES = (
    "resorts",
    "resortHours",
    "resortRates",
...
def push_snapshot(snapshot):
    ...
    for table_name in TABLE_NAMES[1:]:
        table_resort_id, _, _ = require_metadata(contents[table_name], table_name)
    ...
    hours_resort_id, hours_source_url, hours_updated_at = require_metadata(
        hours, "resortHours"
    )
    rates_resort_id, rates_source_url, rates_updated_at = require_metadata(
        rates, "resortRates"
    )
    rentals_resort_id, rentals_source_url, rentals_updated_at = require_metadata(
        rentals, "resortRentals"
    )
    weather_resort_id, weather_source_url, weather_updated_at = require_metadata(
        weather, "weatherAndStatus"
    )
