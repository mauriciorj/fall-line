...
BLUE_MOUNTAIN_DIR = Path(__file__).resolve().parent
CRAWLER_DIR = BLUE_MOUNTAIN_DIR.parents[1]
SNAPSHOT_GENERATOR = BLUE_MOUNTAIN_DIR / "snapshot_generator.py"
SNAPSHOT_FILE = BLUE_MOUNTAIN_DIR / "snapshot_generator.json"
TABLE_NAMES = (
    "resorts",
    "resortHours",
    "resortRates",
...
def push_snapshot(snapshot):
    ...
    return {
        ...
        "resortHours": push_resort_hours(
            hours_id, hours_url, hours["hours"], updated_at_ms=hours_time
        ),
        "resortRates": push_resort_rates(
            rates_id, rates_url, rates["rates"], updated_at_ms=rates_time
        ),
        "resortRentals": push_resort_rentals(
            rentals_id, rentals_url, rentals["rentals"], updated_at_ms=rentals_time
        ),
