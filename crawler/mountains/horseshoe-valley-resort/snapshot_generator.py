...
DIFFICULTY_FIELDS = {
    "easy": "green",
    "difficult": "blue",
    "more_difficult": "black",
    "hard": "black",
    "double black": "doubleBlack",
    "terrain_park": "freeStyle",
    "freestyle": "freeStyle",
}
...
def build_tracks_summary(weather):
    ...
    for trail in weather.get("trails", []):
        ...
        if field:
            summary[field] += 1
    for park in weather.get("terrainParks", []):
        field = DIFFICULTY_FIELDS.get(park.get("difficulty", "").lower())
        if field:
            summary[field] += 1
    return summary
...
def build_resort(static_info, rates, rentals, weather):
    ...
    prices = {
        "dayTicketPrice": get_rate_price(rates, "adult", "closing_weekend"),
        "skiRentalPrice": get_rental_price(
            rentals, "rental", "skiing or snowboarding package", "adult"
        ),
    }
